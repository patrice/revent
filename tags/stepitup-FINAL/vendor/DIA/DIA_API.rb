require "NotImplementedException"
...
class DIA_API
	...
	# this will select a subclass to initialize
	# options - type => type of subclass - "default", "DIA_Simple"
	# 
	#       other options by subclass...
	#        eg. authCode => [user, password, orgkey]
	#            urls => { 'get' => ... }
	def DIA_API.create(options = nil)
	...
	#
	#           String: same as { 'key' => String }
	def process(table, data, options)
	...
	# Not Implemented (I never saw it used anywhere...)
  ...
  #   mainly used for group membership, but theoretically more general. 
	...
	# (just make sure to omit and ()'s in your statement)
	...
	# raises a NotImplementedException at the location of the original
	# call to this library
	def notImplemented(function)
		raise NotImplementedException, "'" + function +"' not implemented in " + self.class.name, caller[1..-1];
	end
