...
# make a directory to hold the views if it isn't already there...
viewdir = to_dir + 'views/' + table
if !File.exists?(viewdir)
	Dir.mkdir(viewdir)
end

files = ['controllers/template_controller.rb', 
					'models/template.rb', 
					'views/template/edit.rhtml', 
					'views/template/list.rhtml', 
					'views/template/show.rhtml', 
					'views/template/new.rhtml', 
					'views/template/_form.rhtml' ]

files.each do |file|
	outfile = to_dir + file.gsub(/template/, table)
